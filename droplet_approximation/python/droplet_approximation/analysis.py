...
def plot_droplet_size_temperatures_score( particle_dataframe, score_report, cluster_filter=None,
                                          max_deviations=1, time_range=None, **kwargs ):
    """
    ...
    Takes 3 Arguments:
      ...
      max_deviations     - Optional integer capping the number of deviations plotted.
                           Defaults to 1. Prioritizes plotting earliest deviations from
    ...
    if hasattr( score_report, "run_deviations" ):
        if not score_report.run_deviations and max_deviations > 0:
            warnings.warn( "Deviation analysis was not run for this score report. " \
                           "Cannot plot deviations. Setting max_deviations to 0." \
                           "Setting cluster filter to None.")
            max_deviations = 0
            cluster_filter = []
    else:
