...
! ----------------------------------------------------------------------
      ...
      ! j_recl = record length in "bytes" for history file
      ! k8     = kind parameter for integers in mpi_io routines
...
! ----------------------------------------------------------------------
      real :: zetas(3), gama(3), etas(4), dt_new, &
          ...
          ucfl, vcfl, wcfl
! ----------------------------------------------------------------------
      character(len=80) :: path_res, path_prt, &
          ...
          path_traj, &
          bad_news
      character(len=3)  :: case_inp
      character(len=80) :: path_viz_xy, path_viz_xz, path_viz_yz, path_stuf, &
                           path_seed
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
      ...
      ! isize tracks total size for history file I/O
      isize = 0
